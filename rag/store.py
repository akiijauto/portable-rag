...
class Store:
    def __init__(self, index_dir, cache_mb=256, log=None):
        ...
        SQLite の既定は 2MB しかなく、転置索引への書き込みが全体を通して遅くなる。
        実測は2通り。
          - postings 8.8M 行の索引へ10件追記: 9.1秒 → 3.1秒（約3倍）
          - 300件103MB を最初から構築: 128秒 → 61秒（2.1倍）

        **これは定数倍の改善であって、「途中から急に遅くなる」現象の説明ではない。**
        所要時間は処理したチャンク数でほぼ決まり（相関 +0.99）、索引が育つことによる
        悪化は 1,333万行の範囲で 26〜33% にとどまる（相関 +0.15〜0.23）。
        """
